...
const API_BASE = (api.defaults.baseURL ?? process.env.NEXT_PUBLIC_API_BASE_URL ?? "").replace(/\/+$/, "");
...
export default function WhatsAppStubTestPage() {
  ...
  const handleSend = async () => {
    ...
    try {
      if (!API_BASE) {
        throw new Error("NEXT_PUBLIC_API_BASE_URL is not configured. Check frontend/.env.local.");
      }
      ...
      await pollTask(newTaskId, token);
    } catch (err) {
  ...
  const pollTask = async (id: string, token: string) => {
    setStatusLabel("Polling");

    if (!API_BASE) {
      setError("NEXT_PUBLIC_API_BASE_URL is not configured. Check frontend/.env.local.");
      setStatusLabel("Error");
      return;
    }
  ...
  const statusColor = (() => {
    switch (statusLabel) {
      ...
      case "Timeout":
        return "bg-amber-700 text-neutral-100";
      default:
  ...
  return (
    <section className="card space-y-6">
      ...
      {error ? (
        <div className="rounded-md border border-red-700 bg-red-950/60 px-4 py-3 text-sm text-red-200">{error}</div>
      ) : null}
